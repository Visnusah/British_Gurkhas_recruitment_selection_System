...
def open_phase2_part2(root):
    # phase2_part2_window colors and fonts
    background = "#2D8A69" 
    framefg = "white" 
    frame_clr= "#DBDBDB"
    ...
    # root.destroy()
    phase2_part2_window = tk.Toplevel(root)
    phase2_part2_window.title("British Gurkhas recruitment process")
    phase2_part2_window.maxsize(1280, 770)
    phase2_part2_window.minsize(1280, 770)

    phase2_part2_window.geometry("1250x700+210+100") # width x height + x_offset + y_offset
    phase2_part2_window.config(bg=background)
    phase2_part2_window.resizable(False, False)
    ...
    def on_next_click():
        army = get_army_data()
        print(army.__dict__)
        print(army.first_emergency_contact.__dict__)
        print(army.second_emergency_contact.__dict__)
        # return user
        pass


    next_btn = ctk.CTkButton(phase2_part2_window, text="Next", width=130, height=45, corner_radius=10, font=next_btn_font, fg_color="#2D8A69", bg_color=frame_clr,command=on_next_click)
