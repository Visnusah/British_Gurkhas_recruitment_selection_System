...
def open_phase2_part1_window(parent):
    
    phase2_part1_window = tk.Toplevel()
    phase2_part1_window.title("British Gurkhas recruitment process")
    phase2_part1_window.maxsize(1280, 700)
    phase2_part1_window.minsize(1280, 700)
    ...
    phase2_part1_window.geometry("1250x700+210+100") # width x height + x_offset + y_offset
    ...
    def on_next_click(window):
        army = get_army_data()
        print(army.__dict__)
        open_phase2_part2(window)
        # window.destroy()
        # return army


    next_btn = ctk.CTkButton(phase2_part1_window, text="Next", width=130, height=45, corner_radius=10, font=next_btn_font, fg_color="#2D8A69", bg_color=frame_clr,command=lambda: on_next_click(phase2_part1_window))
